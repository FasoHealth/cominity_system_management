...
const CAT_LABELS = {
    ...
    suspicious_activity: 'Suspect', fire: 'Incendie', accident: 'Accident', other: 'Autre'
};
const CAT_ICONS = {
...
const DashboardPage = () => {
    ...
    const statCards = [
        { icon: '📢', label: 'Total Signalés', value: stats.total, bg: 'rgba(232,84,26,0.12)', color: 'var(--brand-orange)' },
        { icon: '✅', label: 'Approuvés', value: stats.approved, bg: 'var(--green-bg)', color: 'var(--green)' },
        { icon: '⏳', label: 'En attente', value: stats.pending, bg: 'var(--yellow-bg)', color: 'var(--yellow)' },
        { icon: '🏆', label: 'Résolus', value: stats.resolved, bg: 'var(--blue-bg)', color: 'var(--blue)' },
    ];
    ...
    return (
        <div className="page-container fade-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 340px', gap: 24 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                    ...
                    <div className="card">
                        <div className="card-header">
                            <h3 className="card-title">📈 Signalements par mois</h3>
                        </div>
                                            ...
                                            <tr key={inc._id} onClick={() => window.location.href = `/incidents/${inc._id}`}>
                                                <td>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        <span>{CAT_ICONS[inc.category] || '⚠️'}</span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                    <div className="card">
                        <h3 className="card-title" style={{ marginBottom: 16 }}>⚡ Actions rapides</h3>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            <Link to="/report" className="btn btn-primary btn-full" style={{ padding: '13px 16px', justifyContent: 'flex-start', gap: 10 }}>
                                ➕ <span>Nouveau signalement</span>
                            </Link>
                            <Link to="/notifications" className="btn btn-secondary btn-full" style={{ padding: '13px 16px', justifyContent: 'flex-start', gap: 10 }}>
                                🔔 <span>Voir notifications</span>
                            </Link>
                            <Link to="/map" className="btn btn-secondary btn-full" style={{ padding: '13px 16px', justifyContent: 'flex-start', gap: 10 }}>
                                🗺️ <span>Carte des alertes</span>
                            </Link>
                            <Link to="/feed" className="btn btn-secondary btn-full" style={{ padding: '13px 16px', justifyContent: 'flex-start', gap: 10 }}>
                    ...
                    <div className="card" style={{ background: 'linear-gradient(135deg, var(--brand-navy) 0%, var(--brand-navy-light) 100%)', border: 'none' }}>
                        <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
                            💡 Le saviez-vous ?
                        </p>

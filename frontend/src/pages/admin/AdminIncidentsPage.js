...
const CAT_LABELS = { theft: 'Vol', assault: 'Sécurité', vandalism: 'Vandalisme', suspicious_activity: 'Suspect', fire: 'Incendie', kidnapping: 'Enlèvement', other: 'Autre' };
const CAT_ICONS = { 
    theft: ShieldAlert, 
    assault: ShieldAlert, 
    vandalism: Hammer, 
...
const AdminIncidentsPage = () => {
    ...
    const handleNotifyService = async (id, service) => {
        ...
        } catch (err) { alert(err.response?.data?.message || 'Erreur de notification.'); }
        finally { setActionLoading(null); }
    };
    ...
    const TABS = [
        { v: 'pending', l: 'En attente', c: counts.pending, icon: Clock },
        { v: 'approved', l: 'Approuvés', c: counts.approved, icon: CheckCircle2 },
        { v: 'rejected', l: 'Rejetés', c: counts.rejected, icon: X },
        { v: 'resolved', l: 'Résolus', c: counts.resolved, icon: Trophy },
    ];
        ...
        <div className="page-container fade-in">
            ...
            <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
                    <h1 className="page-title" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 12 }}>
                        <ShieldAlert size={28} color="var(--brand-orange)" /> Modération
                    </h1>
                ...
                {TABS.map(t => {
                    ...
                    return (
                        <button key={t.v} onClick={() => setActiveTab(t.v)} style={{
                            ...
                            borderBottom: `2.5px solid ${activeTab === t.v ? 'var(--brand-orange)' : 'transparent'}`,
                            marginBottom: -1, transition: 'all 0.2s',
                            display: 'flex', alignItems: 'center', gap: 8
                        }}>
                            <TabIcon size={16} />
                            {t.l} 
                            {t.c > 0 && <span style={{ fontSize: '0.7rem', fontWeight: 800, marginLeft: 4, background: activeTab === t.v ? 'rgba(232,84,26,0.1)' : 'var(--bg-secondary)', color: activeTab === t.v ? 'var(--brand-orange)' : 'var(--text-muted)', padding: '2px 8px', borderRadius: 20 }}>{t.c}</span>}
                        </button>
                    );
                })}
            </div>
            ...
            {loading ? (
                            ...
                            <div key={inc._id} className="mod-card fade-in" style={{ borderRadius: 16, border: '1px solid var(--border)', padding: 16, background: 'var(--bg-primary)', display: 'grid', gridTemplateColumns: '80px 1fr auto', gap: 20, alignItems: 'center', transition: 'all 0.2s' }}>
                                ...
                                <div className="mod-card-body">
                                    <div style={{ display: 'flex', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
                                        ...
                                        {inc.status === 'pending' && <span style={{ background: 'rgba(234,179,8,0.1)', color: '#b45309', fontSize: '0.65rem', fontWeight: 800, padding: '2px 10px', borderRadius: 20, border: '1px solid rgba(234,179,8,0.2)' }}>EN ATTENTE</span>}
                                        {inc.upvotes?.length >= 5 && inc.status === 'approved' && <span style={{ background: 'rgba(16,185,129,0.1)', color: '#059669', fontSize: '0.65rem', fontWeight: 800, padding: '2px 10px', borderRadius: 20, border: '1px solid rgba(16,185,129,0.2)' }}>VALIDÉ PAR LA COMMUNAUTÉ</span>}
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', gap: 20, fontSize: '0.75rem', color: 'var(--text-muted)', alignItems: 'center' }}>
                                        ...
                                        {inc.upvoteCount > 0 && (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: 'var(--brand-orange)', fontWeight: 700 }}><ThumbsUp size={12} /> {inc.upvoteCount} votes</div>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    {activeTab === 'pending' && (
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button className="btn btn-sm btn-success" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, height: 36, borderRadius: 8 }}
                                                ...
                                                {actionLoading === `${inc._id}-approved` ? <RefreshCw size={14} className="spin" /> : <Check size={14} />} Approuver
                                            </button>
                                            <button className="btn btn-sm btn-danger" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, height: 36, borderRadius: 8 }}
                                                ...
                                                {actionLoading === `${inc._id}-rejected` ? <RefreshCw size={14} className="spin" /> : <X size={14} />} Rejeter
                                            </button>
                                        </div>
                                    )}
                                    {activeTab === 'approved' && (
                ...
                <div className="card empty-state" style={{ padding: '64px 24px', textAlign: 'center' }}>
                    ...
                    <p className="empty-state-title" style={{ fontSize: '1.2rem', fontWeight: 700 }}>Tout est traité !</p>
                    <p className="empty-state-desc" style={{ color: 'var(--text-muted)', marginTop: 8 }}>Aucun signalement à modérer dans cet onglet.</p>
                </div>
            )}
        </div>
    );
